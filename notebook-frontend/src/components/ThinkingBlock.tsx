...
const ThinkingBlock: React.FC<ThinkingBlockProps> = ({ 
  ...
  const processContent = (rawContent: string) => {
    ...
    // 移除可能的标记
    processed = processed.replace(/^【思考过程】|^\[思考过程\]|^思考过程:|^【AI思考】|^\[AI思考\]|^AI思考:/i, '').trim();
    
    // 移除 "## 思考过程" 或类似的标题行
    processed = processed.replace(/^## 思考过程\s*\n+/i, '').trim();
  ...
  return (
    <div className="thinking-block block-container">
      <div 
        className="block-header-compact"
        onClick={toggleCollapsed}
      >
        <div className="block-title">
          <IconBulb className="block-icon" />
        </div>
      </div>
