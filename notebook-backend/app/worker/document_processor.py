...
class DocumentProcessor:
    ...
    async def process_document(self, document_id: str) -> None:
        ...
        try:
            ...
            # 4. 存储文档文本
            logger.info(f"保存文档文本内容: {document_id}")
            await self.document_service.update_document_content(document_id, text_content)
            
            # 5. 文档向量化处理
            logger.info(f"文档向量化处理: {document_id}")
            vector_ids = await self.vector_store.store_document_vectors(document_id, text_content)
